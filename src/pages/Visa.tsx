...
import { AlertTriangle, Info, ArrowRight, FileText } from "lucide-react";
import { visaData } from "../data/general/visa";
import SectionNav from "../components/SectionNav";

const SECTIONS = [
  { id: "flow", label: "3-Stage Flow" },
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-12">
        ...
        <section id="flow" className="scroll-mt-20">
          <h2 className="text-xl font-bold text-gray-800 mb-1 border-b border-gray-200 pb-2" style={{ fontFamily: "Playfair Display, serif" }}>
            Where the Visa Fits — 3-Stage Flow
          </h2>
          <p className="text-xs text-gray-500 mb-4">The visa is the middle stage. Both the stage before and after require separate action.</p>

          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="bg-indigo-50 border-b border-indigo-100 px-5 py-3">
              <p className="text-xs font-semibold text-indigo-700">Visa type: {visaData.visaFlow.visaType}</p>
            </div>
            <div className="px-5 py-4 space-y-0">
              {visaData.visaFlow.steps.map((step, i) => (
                <div key={step.n} className="flex gap-4">
                  <div className="flex flex-col items-center">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 ${step.n === 2 ? "bg-[#0B1A33] text-white" : "bg-gray-100 text-gray-500"}`}>
                      {step.n}
                    </div>
                    {i < visaData.visaFlow.steps.length - 1 && (
                      <div className="w-px flex-1 bg-gray-200 my-1 min-h-[24px]" />
                    )}
                  </div>
                  <div className="pb-5 flex-1">
                    <div className="flex items-center gap-2 mb-0.5">
                      <p className={`text-sm font-bold ${step.n === 2 ? "text-[#0B1A33]" : "text-gray-600"}`}>
                        {step.label}
                        {step.n === 2 && <span className="ml-2 text-xs font-normal text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded">← this page</span>}
                      </p>
                    </div>
                    <p className="text-xs text-gray-600 leading-relaxed">{step.detail}</p>
                    {step.linkTo && (
                      <Link to={step.linkTo} className="inline-flex items-center gap-1 mt-1.5 text-xs text-indigo-600 hover:text-indigo-800 font-medium">
        ...
        <section id="deadlines" className="scroll-mt-20">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
            <p className="text-xs font-bold text-blue-800 mb-2 flex items-center gap-1.5">
              ...
              University-specific deadline examples
            </p>
            <ul className="space-y-1.5">
              {visaData.keyDeadlines.universityExamples.map((ex, i) => (
        ...
        <section id="financial" className="scroll-mt-20">
          ...
          <div className="space-y-4">
            ...
            {/* University figure caution */}
            <div className="bg-amber-50 border border-amber-300 rounded-xl p-4 flex items-start gap-2">
              <AlertTriangle className="w-4 h-4 text-amber-600 flex-shrink-0 mt-0.5" />
              <p className="text-sm text-amber-900 leading-relaxed">{visaData.financialRequirements.universityFigureWarning}</p>
            </div>
          </div>
        </section>
        ...
        <section id="embassy" className="scroll-mt-20">
          ...
          <div className="space-y-2.5">
            {visaData.embassyAndVfs.additionalNotes.map((note, i) => (
              <div key={i} className="bg-indigo-50 border border-indigo-200 rounded-xl p-4 flex items-start gap-2">
                <Info className="w-4 h-4 text-indigo-600 flex-shrink-0 mt-0.5" />
                <p className="text-sm text-indigo-900 leading-relaxed">
                  {i === 0 ? (
                    <>
                      {note.split("See the Universitaly page")[0]}
                      See the{" "}
                      <Link to="/universitaly" className="underline font-semibold hover:text-indigo-700">
                        Universitaly page
                      </Link>
                      {note.split("See the Universitaly page")[1]}
                    </>
                  ) : note}
                </p>
              </div>
            ...
            <div className="divide-y divide-gray-100">
              {visaData.documentChecklist.items.map(item => (
                <div key={item.num} className="flex gap-4 px-5 py-4">
                  <span className="flex-shrink-0 w-7 h-7 bg-[#C9A227] text-white text-xs font-bold rounded-full flex items-center justify-center mt-0.5">
                    {item.num}
                  </span>
                  <div className="flex-1">
                    ...
                    {item.addendum && (
                      <div className="mt-2 flex items-start gap-1.5 bg-blue-50 border border-blue-100 rounded px-2.5 py-2">
                        <Info className="w-3 h-3 text-blue-500 flex-shrink-0 mt-0.5" />
                        <p className="text-xs text-blue-800 leading-relaxed">{item.addendum}</p>
                      </div>
                    )}
                  </div>
                </div>
        ...
        <section id="permit" className="scroll-mt-20">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              ...
              <div className="mt-3 flex items-start gap-1.5 bg-gray-50 border border-gray-200 rounded px-3 py-2">
                <span className="text-gray-400 text-xs mt-0.5 flex-shrink-0">+</span>
                <p className="text-xs text-gray-600">Plus <strong>€16 revenue stamp (marca da bollo)</strong> — buy 2 if also collecting a certificate at an office.</p>
              </div>
            </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-start gap-2">
              ...
              <p className="text-sm text-blue-900 leading-relaxed">{visaData.permitOfStay.ssnNote}</p>
            </div>

            {/* Biometric appointment note */}
            <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-4 flex items-start gap-2">
              <Info className="w-4 h-4 text-indigo-600 flex-shrink-0 mt-0.5" />
              <p className="text-sm text-indigo-900 leading-relaxed">
                {visaData.permitOfStay.biometricNote.split("Full step-by-step is on the After Arrival page")[0]}
                Full step-by-step is on the{" "}
                <Link to="/arrival" className="underline font-semibold hover:text-indigo-700">After Arrival page</Link>.
              </p>
            </div>
          </div>
        </section>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {visaData.vfsFees.visaFees.map((fee, i) => (
                    <tr key={i} className={`${i % 2 === 0 ? "bg-white" : "bg-gray-50"} ${fee.chargeType === "Long Stay (Study)" ? "font-semibold" : ""}`}>
